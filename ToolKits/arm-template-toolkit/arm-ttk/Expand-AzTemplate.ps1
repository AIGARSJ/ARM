...
"
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'SpecificTemplate') {
            ...
            $resolvedTemplatePath =
                ...
                if ($TemplatePath -notlike '*.json') {
                    ...
                    $preferredJsonFile = $TemplatePath |
                        ...
                        # for a file named azuredeploy.json, prereq.azuredeploy.json, mainTemplate.json or main.json (bicep)
                        Where-Object { 'azuredeploy.json', 'mainTemplate.json', 'prereq.azuredeploy.json', 'main.json' -contains $_.Name } |
                        Select-Object -First 1 -ExpandProperty Fullname
                    # If no file was found, write an error and return.
                    if (-not $preferredJsonFile) {
                        Write-Error "No main template found beneath $TemplatePath"
                        return
                    }
                    $preferredJsonFile
                } else {
            ...
            $isMainTemplate = 'mainTemplate.json', 'azuredeploy.json', 'prereq.azuredeploy.json', 'main.json' -contains $templateFileName
